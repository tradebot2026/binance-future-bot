"""Volatility expansion mean-reversion strategy module."""
...
class VolExpansionStrategy(StrategyModule):
    ...
    def max_concurrent_positions(self) -> int:
        return Config.MAX_VEMR_POSITIONS

    def priority_weight(self) -> float:
        return Config.STRATEGY_PRIORITY_VEMR

    def size_multiplier(self, score: float) -> float:
        if score >= Config.SCORE_FULL_SIZE:
            return 1.0
        if score >= Config.VEMR_MIN_SCORE:
    ...
    def evaluate(self, snapshot: MarketSnapshot) -> Optional[SignalCandidate]:
        ...
        return pick_directional_candidate(
            ...
            min_score_fn=lambda _c, _m: Config.VEMR_MIN_SCORE,
    ...
    def _eval_dir(
        ...
        if atr <= 0:
            return None
        result = evaluate_vol_expansion_mr(action, df_entry, df_confirm, price, atr)
